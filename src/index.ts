...
import { handleTempleApproval, handleTempleSelection, handleTempleModuleOffer } from "./utils/temple-onboarding.js";
...
function getRomeTimeParts(date: Date): { hours: number; minutes: number } {
...
function isAtRomeTime(date: Date, time: string): boolean {
...
function resolveDailyHostMentions(config: any): string[] {
...
function resolveDailyMissionMentions(config: any): string[] {
...
async function triggerDailyForGuild(client: Client, guildId: string, config: any): Promise<void> {
    ...
    const idx = dailyConfigs.findIndex((item: any) => item.guildId === guildId);
    if (idx < 0) return;
...
async function resetDailyForGuild(client: Client, guildId: string, config: any): Promise<void> {
    const cfg = loadConfig();
    const dailyConfigs = Array.isArray(cfg.dailyConfigs) ? cfg.dailyConfigs : [];
    const idx = dailyConfigs.findIndex((item: any) => item.guildId === guildId);
    if (idx < 0) return;
...
export async function startBot(): Promise<void> {
    ...
    const dailyScheduler = async () => {
        ...
        for (const daily of dailyConfigs) {
            ...
            if (daily.lastTriggeredDate === todayKey && !shouldStart) continue;
    ...
    client.on("guildMemberAdd", async (member) => {
        ...
        try {
            ...
            await import("./utils/temple-onboarding.js").then(({ sendTempleModuleOffer }) => sendTempleModuleOffer(member as GuildMember));
        } catch (err) {
    ...
    client.on("interactionCreate", async (interaction: Interaction) => {
        if (interaction.isButton() && interaction.customId.startsWith("temple-onboarding:offer-send:")) {
            await handleTempleModuleOffer(interaction as ButtonInteraction, true);
            return;
        }
        if (interaction.isButton() && interaction.customId.startsWith("temple-onboarding:offer-deny:")) {
            await handleTempleModuleOffer(interaction as ButtonInteraction, false);
            return;
        }
    ...
    client.on("messageCreate", async (message: Message) => {
        ...
        if (guildId) {
            ...
            for (const daily of dailyConfigs) {
                ...
                if (!missionMessageId || !missionsChannelId || message.channel.id !== missionsChannelId) continue;
                if (!message.reference || message.reference.messageId !== missionMessageId) continue;
