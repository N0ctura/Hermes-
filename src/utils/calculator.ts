...
const ORDINALI: Record<string, string> = {
    seconda: "2",
    terza: "3",
    quarta: "4",
    quinta: "5",
...
// Sostituzioni testuali, DA APPLICARE IN QUESTO ORDINE
// (le frasi più lunghe/specifiche prima di quelle corte, altrimenti
// "diviso per" verrebbe già consumato parzialmente da "diviso")
type Sostituente = string | ((...args: string[]) => string);

const SOSTITUZIONI: Array<[RegExp, Sostituente]> = [
...
export interface RisultatoCalcolo {
    ok: boolean;
    espressioneNormalizzata?: string;
    risultato?: number | string;
    errore?: string;
}

export function calcolaEspressione(testoGrezzo: string): RisultatoCalcolo {
