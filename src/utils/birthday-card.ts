...
import type { CardConfig } from "./storage.js";
...
function substituteBirthdayPlaceholders(text: string, data: BirthdayCardData): string {
  let result = text;
 ...
 */
export function getDefaultBirthdayCard(): CardConfig {
  return {
    width: 1440,
    height: 560,
    layers: [
...
export async function generateBirthdayCard(
  ...
  async function loadImageSafe(url: string | undefined): Promise<any | null> {
    if (!url) return null;
    try {
...
export async function generateBirthdayAttachment(
  data: BirthdayCardData,
  cardConfig?: CardConfig
