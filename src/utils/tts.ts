...
const ffmpegStaticPath = ffmpegStaticImport as unknown as string | null;
...
function isBlockedVoiceChannel(guildId: string, voiceChannelId?: string | null): boolean {
...
export async function playTextInChannel(
  ...
  try {
    ...
    logger.error(
      { guildId, err, currentState: connection.state.status },
      "TTS: la connessione vocale NON è diventata pronta entro 20s — probabile blocco di rete (UDP) lato host, non un problema di codice/dipendenze"
    );
    connection.destroy();
    connections.delete(guildId);
    activeVoiceChannels.delete(guildId);
 ...
 */
export async function joinVoiceChannelManual(
  guildId: string,
  voiceChannelId: string,
  client: Client
): Promise<void> {
...
export function getActiveVoiceChannelId(guildId: string): string | undefined {
...
export async function handleMessageForTTS(message: {
  ...
  if (!userVoiceChannelId || !botVoiceChannelId || userVoiceChannelId !== botVoiceChannelId || blockedChannelIds.has(userVoiceChannelId)) return;
