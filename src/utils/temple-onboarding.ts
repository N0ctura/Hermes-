...
import { loadConfig, saveConfig, type GuildTempleOnboardingConfig, type TempleOnboardingRequest, DEFAULT_THRESHOLD_TIERS } from "./storage.js";
...
async function fetchXpForMember(member: GuildMember, config: GuildTempleOnboardingConfig): Promise<number | null> {
  if (!config.fetchXpFromWolvesville) return null;
...
function highestThresholdRoleId(guild: Guild, xp: number | null, templeKey: string): string | null {
...
export async function handleTempleApproval(interaction: ButtonInteraction, approve: boolean): Promise<void> {
  ...
  if (config.assignTempleRole && temple.roleId && !target.roles.cache.has(temple.roleId)) await target.roles.add(temple.roleId, "Temple onboarding: tempio autorizzato").catch((err) => logger.warn({ err }, "Temple onboarding: ruolo tempio non assegnato"));
  ...
  if (xpRoleId && !target.roles.cache.has(xpRoleId)) await target.roles.add(xpRoleId, "Temple onboarding: soglia XP Wolvesville").catch((err) => logger.warn({ err }, "Temple onboarding: ruolo XP non assegnato"));
  ...
  if (config.sendGeneralMessage && config.generalChannelId) await sendToChannel(guild, config.approvedGeneralMessage, await guild.channels.fetch(config.generalChannelId).catch(() => null) as TextChannel | null, target, templeName);
  if (config.sendTempleMessage && temple?.channelId) await sendToChannel(guild, tc?.templeMessage || config.approvedTempleMessage, await guild.channels.fetch(temple.channelId).catch(() => null) as TextChannel | null, target, templeName);
