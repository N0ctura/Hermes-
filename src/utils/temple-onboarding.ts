...
const defaultTempleDetails: Record<string, string> = {
...
function resolveTempleRoleIds(guild: Guild, config: GuildTempleOnboardingConfig, key: string) {
  ...
  const coLeaderRoleIds =
    tc?.coLeaderRoleIds && tc.coLeaderRoleIds.length > 0
      ? tc.coLeaderRoleIds
      : guild.roles.cache
...
/**
 * Ricalcola il conteggio dei Templi usando i membri attuali di Discord.
 * Non usa un conteggio persistito: prima aggiorna la cache dei membri e poi
 * legge nuovamente i membri associati ai ruoli principali dei Templi.
 */
async function refreshTempleMemberCache(guild: Guild): Promise<void> {
  try {
    await guild.members.fetch();
  } catch (err) {
    // Se il fetch completo fallisce, manteniamo la cache disponibile invece
    // di bloccare l'onboarding. Il conteggio viene comunque rifatto subito
    // dopo sulla cache corrente.
    logger.warn({ err, guildId: guild.id }, "Temple onboarding: refresh membri Discord fallito, uso la cache corrente");
  }
}
...
function buildSelectionComponents(
  guild: Guild,
  config: GuildTempleOnboardingConfig,
  memberId: string,
  selectable = getSelectableTemples(guild, config),
) {
...
function roleMentions(guild: Guild, roleIds: string[] | undefined): string {
  return (roleIds ?? []).map((id) => guild.roles.cache.has(id) ? `<@&${id}>` : null).filter(Boolean).join(" e ");
}

function buildTempleDetails(guild: Guild, config: GuildTempleOnboardingConfig, selectableKeys: Set<string>): string {
...
function replaceSelectionVariables(template: string, member: GuildMember, details: string, min: number): string {
  return template
...
export async function sendTempleModuleOffer(member: GuildMember): Promise<void> {
  const config = getConfig(member.guild.id);
  if (!config?.enabled || !config.approvalChannelId) return;
  ...
  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId(`${TEMPLE_OFFER_SEND_PREFIX}${member.id}`).setLabel("Invia modulo").setEmoji("🏛️").setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId(`${TEMPLE_OFFER_DENY_PREFIX}${member.id}`).setLabel("Non inviare").setEmoji("❌").setStyle(ButtonStyle.Secondary),
  );
  await (channel as TextChannel).send({ content: text, components: [row] }).catch((err) => logger.error({ err, guildId: member.guild.id, userId: member.id }, "Temple onboarding: invio offerta modulo fallito"));
}

export async function handleTempleModuleOffer(interaction: ButtonInteraction, send: boolean): Promise<void> {
  ...
  await (selectionChannel as TextChannel).send({ content: text, components: [menu] }).catch((err) => logger.error({ err, guildId: guild.id, userId: target.id }, "Temple onboarding: invio modulo fallito"));
...
async function removeOtherTempleRoles(member: GuildMember, config: GuildTempleOnboardingConfig, selectedKey: string) {
  const roleIds = config.temples.flatMap((t) => [t.roleId, ...(t.roleIds ?? [])]).filter((id): id is string => Boolean(id));
  const selected = config.temples.find((t) => t.key === selectedKey);
  const selectedIds = new Set([selected?.roleId, ...(selected?.roleIds ?? [])].filter((id): id is string => Boolean(id)));
  const toRemove = roleIds.filter((id) => !selectedIds.has(id) && member.roles.cache.has(id));
  if (toRemove.length) await member.roles.remove(toRemove, "Temple onboarding: pulizia ruoli altri Templi").catch(() => null);
}

async function addConfiguredTempleRoles(member: GuildMember, config: GuildTempleOnboardingConfig, templeKey: string): Promise<string[]> {
  const temple = config.temples.find((t) => t.key === templeKey);
  if (!temple) throw new Error("Configurazione Tempio non trovata");
  const ids = [temple.roleId, ...(temple.roleIds ?? [])].filter((id): id is string => Boolean(id));
  if (config.assignTempleRole !== false && !temple.roleId) throw new Error("Ruolo principale del Tempio non configurato");
  if (!ids.length) return [];
  const me = member.guild.members.me ?? await member.guild.members.fetchMe().catch(() => null);
  if (!me) throw new Error("Impossibile recuperare il membro Hermes");
  const roles = (await Promise.all(ids.map((id) => member.guild.roles.fetch(id).catch(() => null)))).filter((r): r is NonNullable<typeof r> => Boolean(r));
  if (roles.length !== ids.length) throw new Error("Uno o più ruoli configurati non esistono più");
  for (const role of roles) {
    if (role.managed) throw new Error(`Il ruolo ${role.name} è gestito da un'integrazione`);
    if (role.position >= me.roles.highest.position) throw new Error(`Hermes non può assegnare ${role.name}: sposta il ruolo di Hermes sopra questo ruolo`);
  }
...
export async function handleTempleSelection(interaction: StringSelectMenuInteraction): Promise<void> {
  ...
  const request: TempleOnboardingRequest = { id: crypto.randomUUID(), guildId: guild.id, userId: interaction.user.id, username: interaction.user.username, templeKey: key, status: "pending", createdAt: new Date().toISOString() };
...
export async function handleTempleApproval(interaction: ButtonInteraction, approve: boolean): Promise<void> {
  ...
  const currentSelectable = await getFreshSelectableTemples(guild, config);
  if (!currentSelectable.some((t) => t.key === request.templeKey)) {
    request.status = "denied"; request.resolvedAt = new Date().toISOString(); request.resolvedBy = interaction.user.id; persistConfig(config);
    await interaction.update({ content: "⚠️ La richiesta non può essere autorizzata perché nel frattempo l'equilibrio dei Templi è cambiato.", embeds: interaction.message.embeds, components: [] }); return;
  }
  const temple = config.temples.find((t) => t.key === request.templeKey);
  if (!temple) { await interaction.reply({ content: "❌ Configurazione del Tempio non trovata.", ephemeral: true }); return; }
  ...
  request.status = "approved"; request.resolvedAt = new Date().toISOString(); request.resolvedBy = interaction.user.id; persistConfig(config);
  ...
  if (config.sendGeneralMessage && config.generalChannelId) await sendToChannel(generalMessage, await guild.channels.fetch(config.generalChannelId).catch(() => null) as TextChannel | null, target, templeName);
  if (config.sendTempleMessage && temple.channelId) await sendToChannel(tc?.templeMessage || config.approvedTempleMessage, await guild.channels.fetch(temple.channelId).catch(() => null) as TextChannel | null, target, templeName);
