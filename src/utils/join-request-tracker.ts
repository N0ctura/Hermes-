...
import { loadConfig, saveConfig, type JoinRequestEntry, DEFAULT_JOIN_REQUEST_MESSAGE_TEMPLATE } from "./storage.js";
import { fetchClanLog, fetchPlayerByUsername, JOIN_REQUEST_LOG_ACTION, type WvClanLogEntry } from "./wolvesville.js";
 ...
 */
function pseudoId(entry: WvClanLogEntry, username: string): string {
  return `${entry.creationTime}|${username}`;
}

function requesterUsername(entry: WvClanLogEntry): string | undefined {
  return entry.playerUsername || entry.targetPlayerUsername || undefined;
}

function requesterPlayerId(entry: WvClanLogEntry): string | undefined {
  return entry.playerId || entry.targetPlayerId || undefined;
}

function buildMessageContent(template: string | undefined, username: string, mentionRoleIds: string[] | undefined): string {
...
async function buildProfileCardAttachment(
  username: string,
  guildId: string
): Promise<{ attachment: AttachmentBuilder; embed: EmbedBuilder } | null> {
  try {
    ...
    if (player.personalMessage) embed.setDescription(`*"${player.personalMessage}"*`);
...
async function pollOnce(client: Client): Promise<void> {
  ...
  try {
    ...
    for (const entry of sorted) {
      ...
      if (lastProcessedAt && entryMs < new Date(lastProcessedAt).getTime()) continue;
      ...
      for (const guildConfig of guildConfigs) {
        ...
        if (!channel || !channel.isTextBased()) {
          logger.warn(
            { guildId: guildConfig.guildId, channelId: guildConfig.channelId },
            "join-request-tracker: canale configurato non trovato o non testuale"
          );
          continue;
        }
        ...
        try {
          const sendPayload: Parameters<TextChannel["send"]>[0] = {
            content,
            allowedMentions: { roles: guildConfig.mentionRoleIds ?? [] },
          };
          if (card) {
            (sendPayload as any).embeds = [card.embed];
            (sendPayload as any).files = [card.attachment];
          }
          const msg = await (channel as TextChannel).send(sendPayload);
          notificationMessageId = msg.id;
          notificationChannelId = channel.id;
          logger.info(
            { username, guildId: guildConfig.guildId, channelId: channel.id },
            "join-request-tracker: notifica inviata"
          );
        } catch (err) {
          logger.error(
            { err, username, guildId: guildConfig.guildId, channelId: guildConfig.channelId },
            "join-request-tracker: invio notifica fallito"
          );
        }
      }
    ...
    if (isFirstRun && newHistory.length > 0) {
      logger.info(
        { syncedCount: newHistory.length },
        "join-request-tracker: primo avvio — cursore sincronizzato senza notificare lo storico pregresso"
      );
    }
...
export function startJoinRequestTracker(client: Client): void {
...
export function stopJoinRequestTracker(): void {
  if (pollTimer !== null) {
