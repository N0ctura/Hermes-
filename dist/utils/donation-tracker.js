...
import { fetchClanLedger, fetchClanMembers } from "./wolvesville.js";
 ...
 */
function getDonationAmount(tx) {
    if (!DONATION_TYPES.has(tx.type))
        return null;
    const gold = typeof tx.gold === "number" ? tx.gold : 0;
    const gems = typeof tx.gems === "number" ? tx.gems : 0;
    if (gold > 0)
        return { currency: "gold", amount: gold };
    if (gems > 0)
        return { currency: "gems", amount: gems };
    return null;
}
function isDonation(tx) {
...
async function refreshMembersCache(clanId, force = false) {
    const now = Date.now();
...
function resolvePrimaryGuild(client) {
    ...
    const ids = new Set();
    for (const id of config.notifyChannelIds ?? [])
        ids.add(id);
    if (config.pollChannelId)
        ids.add(config.pollChannelId);
    if (config.donationTracking?.fallbackChannelId)
        ids.add(config.donationTracking.fallbackChannelId);
...
async function resolveDonationChannel(client, templeKey) {
    ...
    if (guild) {
        ...
        if (templeKey) {
            ...
            if (direct)
                return { channel: direct, templeResolved: templeKey };
        }
    }
    ...
    if (fallbackId) {
        ...
        if (ch && ch.isTextBased())
            return { channel: ch, templeResolved: templeKey };
    }
    if (guild && templeKey === null) {
        ...
        if (anyNotify) {
            ...
            if (ch && ch.isTextBased())
                return { channel: ch, templeResolved: null };
        }
    }
...
async function sendDonationNotification(channel, wvUsername, amount, currency, templeLabel, comment) {
...
function transactionToDonationEntry(tx, donation, notificationMessageId, notificationChannelId) {
    if (!tx.playerUsername)
        return null;
    return {
        id: tx.id,
...
async function pollOnce(client) {
    ...
    try {
        ...
        for (const tx of sorted) {
            ...
            if (!wvMember) {
                logger.info({ wvUsername, playerId: tx.playerId }, "donation-tracker: membro non trovato nella lista clan (flair non disponibile, temple via fallback)");
            }
            else {
                logger.info({ wvUsername, flair, templeKey }, "donation-tracker: flair -> tempio risolto");
            }
            ...
            if (!alreadyNotified.has(tx.id)) {
                ...
                if (channel) {
                    const templeLabel = templeResolved
                        ...
                        : null;
                    notificationMessageId = await sendDonationNotification(channel, wvUsername, donation.amount, donation.currency, templeLabel, comment);
...
const TEMPLE_DISPLAY_NAME = new Map([
    ["rinascita", "Rinascita"],
    ["abisso", "Abissi"],
    ["eclissi", "Eclissi"],
    ["folgori", "Folgori"],
]);
export function startDonationTracker(client) {
    if (pollTimer !== null)
        return;
...
export function stopDonationTracker() {
    if (pollTimer !== null) {
...
//# sourceMappingURL=donation-tracker.js.map