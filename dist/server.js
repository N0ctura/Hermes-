...
export async function startWebServer(discordClient) {
    ...
    function ensureGuild(arr, guildId, guildName) {
    ...
    function ensureGuildTts(arr, guildId, guildName) {
        const found = arr?.find((c) => c.guildId === guildId);
        if (found)
            return found;
    ...
    function ensureGuildJoinRequests(arr, guildId, guildName) {
        const found = arr?.find((c) => c.guildId === guildId);
        if (found)
            return found;
        return { guildId, guildName, enabled: false, mentionRoleIds: [] };
    }
    function ensureGuildProfileCard(arr, guildId, guildName) {
        const found = arr?.find((c) => c.guildId === guildId);
        if (found)
            return found;
        return { guildId, guildName };
    }
    function ensureGuildDaily(arr, guildId, guildName) {
        const found = arr?.find((c) => c.guildId === guildId);
        if (found)
            return found;
        return {
    ...
    app.put("/api/module/welcome-leave/:guildId", async (req, res) => {
        ...
        const updated = patchConfig((c) => {
            ...
            const next = { ...current, guildId, guildName: gn, ...incoming };
            if (idx >= 0)
                arr[idx] = next;
            else
                arr.push(next);
    ...
    app.post("/api/module/birthday/:guildId/entries", async (req, res) => {
        const guildId = req.params.guildId;
        const gn = await getGuildName(guildId);
        const { userId, username, day, month } = (req.body || {});
        if (!userId || !day || !month) {
    ...
    app.put("/api/scheduled-messages/:guildId", (req, res) => {
        ...
        const updated = patchConfig((c) => {
            ...
            const base = {
                ...
                recurrenceInterval: incoming.recurrenceInterval,
    ...
    app.delete("/api/scheduled-messages/:guildId/:id", (req, res) => {
        patchConfig((c) => {
            c.scheduledMessages = (c.scheduledMessages || []).filter((m) => !(m.guildId === req.params.guildId && m.id === req.params.id));
        });
        res.status(204).end();
    });
    /* ===== Auto responses ===== */
    ...
    app.put("/api/auto-responses/:guildId", (req, res) => {
        const guildId = req.params.guildId;
        const incoming = (req.body || {});
        if (!incoming.id || !incoming.trigger?.trim())
            return res.status(400).json({ error: "id e trigger required" });
        const updated = patchConfig((c) => {
    ...
    app.delete("/api/auto-responses/:guildId/:id", (req, res) => {
        patchConfig((c) => {
            c.autoResponses = (c.autoResponses || []).filter((response) => !(response.guildId === req.params.guildId && response.id === req.params.id));
        });
        res.status(204).end();
    });
    /* ===== Clan Wolvesville ===== */
    ...
    app.get("/api/clan/donations", (_req, res) => {
        ...
        const currencyOf = (e) => e.currency ?? "gold";
        ...
        for (const d of history) {
            ...
            const cur = donorMap.get(key) || { username: d.playerUsername, totalGold: 0, totalGems: 0, count: 0, lastTime: d.eventTime };
            ...
            cur.count += 1;
            if (new Date(d.eventTime).getTime() > new Date(cur.lastTime).getTime())
                cur.lastTime = d.eventTime;
