...
export const data = new SlashCommandBuilder()
    ...
    .addIntegerOption((opt) => opt.setName("giorno").setDescription("Giorno (1-31)").setMinValue(1).setMaxValue(31).setRequired(true))
    .addIntegerOption((opt) => opt.setName("mese").setDescription("Mese (1-12)").setMinValue(1).setMaxValue(12).setRequired(true)));
export async function execute(interaction) {
    ...
    const guildConfig = idx >= 0 ? { ...arr[idx] } : { guildId, guildName, birthdays: [] };
    ...
    if (entryIdx >= 0)
        guildConfig.birthdays[entryIdx] = entry;
    else
        guildConfig.birthdays.push(entry);
    if (idx >= 0)
        arr[idx] = guildConfig;
    else
        arr.push(guildConfig);
