import { SlashCommandBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, PermissionFlagsBits, ChannelType, AttachmentBuilder, } from "discord.js";
...
function generateLobbyMessage(lobby) {
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: `✅ Partecipanti (${lobby.participants.length})`, value: participantsList, inline: false }, { name: `📋 Riserve (${lobby.reserves.length})`, value: reservesList, inline: false }, { name: `✖ Rimossi (${lobby.removedParticipants.length})`, value: removedList, inline: false })
    ...
    const row = new ActionRowBuilder().addComponents(joinButton, reserveButton, leaveButton);
...
export const data = new SlashCommandBuilder()
    ...
    .addStringOption((opt) => opt.setName("messaggio").setDescription("Messaggio personalizzato da mostrare sopra la lista").setRequired(false))
...
export async function execute(interaction) {
    ...
    if (!targetChannel) {
        await interaction.editReply({
            content: "❌ **Canale rose non configurato!**\n\n" +
    ...
    const lobbyMessage = await targetChannel.send(messageData);
...
export async function handleButtonInteraction(interaction) {
    ...
    const participant = { userId, username, joinedAt: now };
    if (interaction.customId === "rose_join") {
        ...
        if (existingRemovedIdx !== -1)
            lobby.removedParticipants.splice(existingRemovedIdx, 1);
        ...
        if (existingParticipantIdx !== -1)
            lobby.participants.splice(existingParticipantIdx, 1);
        if (existingReserveIdx !== -1)
            lobby.reserves.splice(existingReserveIdx, 1);
        if (existingRemovedIdx === -1)
            lobby.removedParticipants.push(participant);
