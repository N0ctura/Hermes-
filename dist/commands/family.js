import { ActionRowBuilder, EmbedBuilder, PermissionFlagsBits, SlashCommandBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, } from "discord.js";
...
function buildEmbeds(title, description, sections) {
    const embeds = [];
...
function buildTempleData(guild, humans) {
    ...
    return TEMPLE_DEFINITIONS.map((definition) => {
        ...
        const coLeaderRoleIds = guild.roles.cache
            .filter((role) => definition.coLeaderRoleNames.some((coLeaderRoleName) => normalize(role.name) === normalize(coLeaderRoleName)))
            .map((role) => role.id);
        const membersInTemple = humans
            .filter((member) => member.roles.cache.some((role) => isTempleMemberRole(role, definition)))
            .sort(compareMembers);
        const coLeaderIdSet = new Set(membersInTemple
...
function buildMenuRow(temples) {
    const templeOptions = temples.slice(0, 22).map((temple) => new StringSelectMenuOptionBuilder()
        .setLabel(temple.label.slice(0, 100))
        .setValue(`temple:${temple.key}`)
        .setDescription(`${temple.coLeaders.length} co capi, ${temple.members.length} membri`.slice(0, 100)));
    return new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
        ...
        .setLabel("Panoramica")
        .setValue("overview")
        .setDescription("Conteggi generali del server"), new StringSelectMenuOptionBuilder()
        .setLabel("Tutti i membri")
        .setValue("all-members")
        .setDescription("Membri normali con soglia XP"), new StringSelectMenuOptionBuilder()
        .setLabel("Pellegrini")
        .setValue("pilgrims")
        .setDescription("Ospiti con ruolo pellegrini"), ...templeOptions));
}
function buildOverviewEmbeds(humans, pilgrims, temples) {
    const members = humans.filter((member) => !pilgrims.some((pilgrim) => pilgrim.id === member.id));
    const templeLines = temples.map((temple) => `• **${temple.label}** — ${temple.coLeaders.length} co capi, ${temple.members.length} membri`);
    return buildEmbeds("Family", [
...
function buildMembersEmbeds(humans, pilgrims) {
    ...
    return buildEmbeds("Family — Tutti i membri", [
...
function buildPilgrimsEmbeds(pilgrims, pilgrimRoleName) {
    const sortedPilgrims = [...pilgrims].sort(compareMembers);
    return buildEmbeds("Family — Pellegrini", [
...
function buildTempleEmbeds(temple) {
    return buildEmbeds(`Family — ${temple.label}`, [
        `**Tempio:** ${temple.label}`,
...
function parseView(value) {
    ...
    if (value === "pilgrims")
        return { type: "pilgrims" };
    if (value.startsWith("temple:"))
        return { type: "temple", templeKey: value.slice("temple:".length) };
...
export async function execute(interaction) {
    ...
    try {
        ...
        await interaction.editReply("❌ Impossibile fetchare i membri. Controlla che l'intent **GuildMembers** sia attivo nel Developer Portal del bot.");
        return;
    }
    const config = loadConfig();
    const humans = guild.members.cache.filter((member) => !member.user.bot).toJSON();
    const temples = buildTempleData(guild, humans);
    const templeMemberIds = new Set(temples.flatMap((temple) => [...temple.coLeaders, ...temple.members].map((member) => member.id)));
