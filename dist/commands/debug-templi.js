...
export async function execute(interaction) {
    ...
    if (!guild) {
        await interaction.editReply("❌ Solo in un server.");
        return;
    }
    try {
    ...
    const templeRoleNames = [];
    const thresholdRoleNames = [];
    const leaderRoleNames = [];
    ...
    if (thresholdRoleNames.length === 0)
        lines.push("Nessun ruolo soglia trovato nel server.");
    lines.push(``, `**Totale match ruolo↔canale: ${matchCount}**`, `✅ Archivio ruoli aggiornato nella config.`);
    if (matchCount === 0) {
        lines.push(``, `⚠️ **Nessun match trovato.** Entrambi i nomi (ruolo e canale) vengono normalizzati prima del confronto: minuscolo, no accenti, no emoji, caratteri bold Unicode → ASCII.`, `Esempio atteso: ruolo \`♢🔱𝐓𝐞𝐦𝐩𝐢𝐨-𝐝𝐞𝐠𝐥𝐢-𝐀𝐛𝐢𝐬𝐬𝐢\` → canale \`#tempio-degli-abissi\` (entrambi → \`tempioabissi\`)`);
    }
...
//# sourceMappingURL=debug-templi.js.map