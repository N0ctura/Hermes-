    ...
    try {
        ...
        const showCurrentConfig = () => {
            ...
            const validPilgrimRoleId = config.pilgrimRoleId && guild.roles.cache.has(config.pilgrimRoleId)
                ...
                : undefined;
            const durLabel = config.pollDurationHours && config.pollDurationHours > 0
        ...
        const pollSelectRow = new ActionRowBuilder().addComponents(new ChannelSelectMenuBuilder()
            .setCustomId("select_poll_channel")
            .setPlaceholder("Scegli il canale per i sondaggi…")
            .addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement));
        const notifySelectRow = new ActionRowBuilder().addComponents(new ChannelSelectMenuBuilder()
            .setCustomId("select_notify_channels")
            .setPlaceholder("Scegli i canali per le notifiche… (max 10)")
            .setMinValues(1)
            .setMaxValues(10)
            .addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement));
        const durationSelectRow = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
            .setCustomId("select_duration")
            .setPlaceholder("Scegli la durata del sondaggio…")
            .addOptions(DURATION_OPTIONS.map((o) => new StringSelectMenuOptionBuilder().setLabel(o.label).setValue(o.value))));
        const roleSelectRow = new ActionRowBuilder().addComponents(new RoleSelectMenuBuilder()
            .setCustomId("select_role")
            .setPlaceholder("Scegli il ruolo da pingare alla chiusura…"));
        const pilgrimRoleSelectRow = new ActionRowBuilder().addComponents(new RoleSelectMenuBuilder()
            .setCustomId("select_pilgrim_role")
            .setPlaceholder("Scegli il ruolo usato per i pellegrini…"));
        const buildMessageButtons = () => new ActionRowBuilder().addComponents(...MESSAGE_KEYS.map((m) => new ButtonBuilder()
        ...
        const buildStep6Embed = () => {
            ...
            const lines = MESSAGE_KEYS.map((m) => `${m.emoji} **${m.label}:**\n> ${msgs[m.key].slice(0, 100)}${msgs[m.key].length > 100 ? "…" : ""}`);
        ...
        collector.on("collect", async (i) => {
            try {
                if (i.isChannelSelectMenu()) {
                        ...
                        await i.update({
                            embeds: [
                                new EmbedBuilder()
                                    ...
                                    .setDescription(`✅ Canale sondaggi: ${pollChannelMention}\n\n` +
                        ...
                        await i.update({
                            embeds: [
                                new EmbedBuilder()
                                    ...
                                    .setDescription(`✅ Canali notifica: **${notifyChannelsMention}**\n\n` +
                        ...
                        await i.update({
                            embeds: [
                                new EmbedBuilder()
                                    ...
                                    .setDescription(`✅ Ruolo da pingare: ${pingRoleMention}\n\n` +
                        ...
                        await i.update({
                            embeds: [
                                new EmbedBuilder()
                                    ...
                                    .setDescription(`✅ Durata sondaggio: **${durLabel}**\n\n` +
                    ...
                    const msgKey = i.customId.replace("msg_", "");
                    ...
                    try {
                        ...
                        try {
                            await interaction.editReply({ embeds: [buildStep6Embed()], components: [buildMessageButtons()] });
                        }
                        catch { /* ignorato */ }
                    }
                }
