    ...
    return <div className="space-y-5 animate-fade-in">
        ...
        <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-5 space-y-3">
            <div className="flex items-center justify-between gap-3">
                <h3 className="font-bold">Debug snapshot live</h3>
                <div className="text-[11px] text-neutral-400">Guild: {conf.guildId || "—"}</div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
                {(conf.population ?? []).map((pop) => {
                    const temple = conf.temples.find((t) => t.key === pop.key);
                    return <div key={pop.key} className="bg-neutral-950 border border-neutral-800 rounded-xl p-3">
                        <div className="flex items-center justify-between gap-2">
                            <div className="font-bold text-sm">{pop.displayName}</div>
                            <span className={`text-[10px] px-2 py-1 rounded-full ${pop.enabled ? "bg-emerald-500/10 text-emerald-300" : "bg-neutral-700 text-neutral-300"}`}>{pop.enabled ? "attivo" : "off"}</span>
                        </div>
                        <div className="mt-3 text-2xl font-black">{pop.count}</div>
                        <div className="text-[11px] text-neutral-400 mt-1">membri effettivi</div>
                        <div className="mt-3 text-[10px] text-neutral-500 break-all">roleId: {pop.roleId || "—"}</div>
                        <div className="text-[10px] text-neutral-500 break-all">ruoli co-capo: {(temple?.coLeaderRoleIds ?? []).length ? (temple?.coLeaderRoleIds ?? []).join(", ") : "—"}</div>
                    </div>;
                })}
            </div>
        </div>
